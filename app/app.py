...
def cmd_check(args) -> int:
    ...
    for _, image in tag_check_images:
        ...
        if not upstream_tag_names and current_tags:
            ...
            rate_limited = True
            print(f"  ⚠ {name}: no tags returned (rate limited?) — stopping", file=sys.stderr)
            continue

        # No data returned and no existing tags — likely a 404 (bad namespace/image).
        # Mark as error so it can be triaged separately. Bump lastChecked so it
        # doesn't hog the front of the queue on every run.
        if not upstream_tag_names:
            now_iso = datetime.now(timezone.utc).isoformat()
            img_state["lastChecked"] = now_iso
            img_state["checkStatus"] = "error"
            img_state["checkError"] = f"no tags found for {tag_namespace}/{tag_image}"
            ...
            print(f"  ✗ {name}: no tags found ({tag_namespace}/{tag_image}) — marked as error", file=sys.stderr)
            continue
