...
def _get_dockerhub_tags(namespace: str, image: str) -> List[str]:
    ...
    result = _get_dockerhub_tags_rich(namespace, image)
    return [t["name"] for t in result.get("tags", [])]


def _get_dockerhub_tags_rich(namespace: str, image: str) -> Dict:
    """Fetch all tags for a Docker Hub image with digest and timestamp metadata.

    Returns a dict::

        {
            "tags": [{"name": "3.3", "digest": "sha256:...", "last_updated": "ISO-8601"}, ...],
            "error": None | "not_found" | "rate_limited" | "network_error",
            "http_status": None | 404 | 429 | ...,
        }
    ...
    try:
        ...
        code = exc.code
        if code == 404:
            logger.warning(f"Not found: {namespace}/{image} (404)")
            return {"tags": [], "error": "not_found", "http_status": 404}
        elif code == 429:
            logger.warning(f"Rate limited fetching {namespace}/{image} (429)")
            return {"tags": tags, "error": "rate_limited", "http_status": 429}
        else:
            logger.warning(f"HTTP {code} fetching tags for {namespace}/{image}")
            return {"tags": tags, "error": "network_error", "http_status": code}
    except Exception as exc:
        logger.warning(f"Could not fetch tags for {namespace}/{image}: {exc}")
        return {"tags": tags, "error": "network_error", "http_status": None}
...
def cmd_check(args) -> int:
    ...
    for image in resolved_images:
        ...
        state = {
            ...
            "lastChecked": existing.get("lastChecked") if existing else None,
    ...
    for _, image in tag_check_images:
        ...
        # Explicit 404 — bad namespace/image in images.yaml. Mark as error
        # and bump lastChecked so it doesn't hog the queue.
        if fetch_error == "not_found":
            now_iso = datetime.now(timezone.utc).isoformat()
            img_state["lastChecked"] = now_iso
            img_state["checkStatus"] = "error"
            img_state["checkError"] = f"404 not found: {tag_namespace}/{tag_image}"
            with open(img_state_file, "w") as f:
                f.write("# Auto-generated by CascadeGuard\n")
                yaml.dump(img_state, f, default_flow_style=False, sort_keys=False)
            print(f"  ✗ {name}: 404 not found ({tag_namespace}/{tag_image}) — marked as error", file=sys.stderr)
            continue

        # Rate limited — stop processing more images
        if fetch_error == "rate_limited" or (not upstream_tag_names and current_tags):
            rate_limited = True
            print(f"  ⚠ {name}: rate limited — stopping", file=sys.stderr)
            continue

        # Other network error or empty response — skip without marking as permanent error
        if not upstream_tag_names:
            if fetch_error:
                print(f"  ⚠ {name}: {fetch_error} — skipping", file=sys.stderr)
            continue
        ...
        img_state.pop("checkStatus", None)
        img_state.pop("checkError", None)
        ...
        # Compare against state file (known tags), not images.yaml
        actually_new = stable_tag_names - known_tag_names

        # Log concise summary
        print(f"  {name}: {len(stable_tag_names)} stable tags", end="", file=sys.stderr)
        if new_tag_count:
            print(f", {new_tag_count} new", end="", file=sys.stderr)
        if repointed_count:
            print(f", {repointed_count} repointed", end="", file=sys.stderr)
        print("", file=sys.stderr)

        # Only surface genuinely new tags in results
        if actually_new:
            results.append({
                "image": name,
                "status": "new-tags",
