...
def _write_app_image_state(
    ...
    lines = [
        "# Auto-generated by CascadeGuard",
...
"""
      ...
      - name: Apply patch {patch}
        run: git apply _cascadeguard/{patch_path}
"""
      ...
      - name: Read version
        id: version
        run: echo "version=$(cat _cascadeguard/{version_path})" >> "$GITHUB_OUTPUT"

      - name: Resolve tag
        id: tag
        run: |
          version=$(cat _cascadeguard/{version_path})
          if [ -n "${{{{ inputs.suffix }}}}" ]; then
            echo "tag=$version-${{{{ inputs.suffix }}}}" >> "$GITHUB_OUTPUT"
          else
            echo "tag=$version" >> "$GITHUB_OUTPUT"
...
"""
      ...
      - name: Bump version
        ...
        run: |
          ...
          new_version="$major.$minor.$patch"
          echo "$new_version" > _cascadeguard/{version_path}
          cd _cascadeguard
          git config user.name "github-actions[bot]"
          git config user.email "github-actions[bot]@users.noreply.github.com"
          git add {version_path}
...
def main():
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Cache directory for cloned repos (default: .cascadeguard-cache in output dir)",
    )
    ...
    if args.cache_dir:
        ...
        cache_dir = args.output_dir / ".cascadeguard-cache"
